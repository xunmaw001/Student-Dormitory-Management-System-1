            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑宿管</li>
                    </ol>
                </div>
            </div>
        </div>
                    ...
                    <div class="widget-area-2 lochana-box-shadow">
                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                    ...
                                    <div class="form-group col-md-6 suguanNameDiv">
                                        <label>宿管姓名</label>
                                        <input style="width: 450px" id="suguanName" name="suguanName" class="form-control"
                                               placeholder="宿管姓名">
                                    </div>
                                    <div class="form-group col-md-6 suguanPhoneDiv">
                                        <label>宿管手机号</label>
                                        <input style="width: 450px" id="suguanPhone" name="suguanPhone" class="form-control"
                                               onchange="suguanPhoneChickValue(this)"  placeholder="宿管手机号">
                                    </div>
                                    <div class="form-group col-md-6 suguanPhotoDiv">
                                        <label>宿管头像</label>
                                        <img id="suguanPhotoImg" src="" width="100" height="100">
                                        <input name="file" type="file" id="suguanPhotoupload"
                                               class="form-control-file">
                                        <input name="suguanPhoto" id="suguanPhoto-input" type="hidden">
                                    </div>
                                    ...
                                    <div class="form-group col-md-6 suguanEmailDiv">
                                        <label>电子邮箱</label>
                                        <input style="width: 450px" id="suguanEmail" name="suguanEmail" class="form-control"
                                               placeholder="电子邮箱">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        <!-- 当前表的文件上传 -->
        $('#suguanPhotoupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                var photoUrl= baseUrl + 'file/download?fileName=' + data.result.file;
                document.getElementById('suguanPhotoImg').setAttribute('src',photoUrl);
                document.getElementById('suguanPhoto-input').setAttribute('value',photoUrl);
            }
        });
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("suguan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0){
                    window.sessionStorage.removeItem('addsuguan');
    ...
    //数字检查
        <!-- 当前表的数字检查 -->
        function suguanPhoneChickValue(e){
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                password: "required",
                suguanName: "required",
                suguanPhone: "required",
                suguanPhoto: "required",
                sexTypes: "required",
                louyuTypes: "required",
                suguanEmail: "required",
            },
            messages: {
                username: "账户不能为空",
                password: "密码不能为空",
                suguanName: "宿管姓名不能为空",
                suguanPhone: "宿管手机号不能为空",
                suguanPhoto: "宿管头像不能为空",
                sexTypes: "性别不能为空",
                louyuTypes: "楼宇不能为空",
                suguanEmail: "电子邮箱不能为空",
            }
        }).form();
    }
    ...
    // 获取当前详情
    function getDetails() {
        var addsuguan = window.sessionStorage.getItem("addsuguan");
        if (addsuguan != null && addsuguan != "" && addsuguan != "null") {
            //注册表单验证
            $(validform());
            ...
            window.sessionStorage.removeItem('updateId');
            http("suguan/info/" + updateId, "GET", {}, (res) => {
                if(res.code == 0)
                {
                    ruleForm = res.data
                    // 是/否下拉框回显
    ...
    function readonly() {
        if (window.sessionStorage.getItem('role') == '管理员') {
            ...
            $('#louyuTypesSelect').attr('style', 'pointer-events:none;width:450px;');
        }
        else{
            // alert("未知情况.......");
